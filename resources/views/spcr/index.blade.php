...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">SPCR Management</h1>
            <p class="text-xs text-gray-400 font-medium">Strategic Performance Commitment and Review</p>
        </div>
        <div class="flex gap-2 items-center">
            ...
            <button class="btn btn-orange flex items-center gap-2 shadow-sm" onclick="createSpcr()">
                <i class="fas fa-plus"></i> Create SPCR
            </button>
        </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <!-- Profile Card / Division Oversight -->
        <div class="lg:col-span-1">
            <div class="card-modern p-8 flex flex-col items-center text-center relative overflow-hidden">
                ...
                <h2 class="text-xl font-bold text-gray-900 mb-1">{{ Auth::user()->division_name }}</h2>
                <p class="text-[10px] font-black text-orange-500 uppercase tracking-[0.2em] mb-6">DIVISION OVERSIGHT</p>
                ...
                <div class="w-full space-y-4 pt-6 mt-2 border-t border-gray-100">
                    <div class="flex items-center gap-4 bg-gray-50/50 p-3 rounded-2xl border border-gray-50/50">
                        ...
                        <div class="text-left">
                            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Division Head</p>
                    ...
                    <div class="flex items-center gap-4 bg-gray-50/50 p-3 rounded-2xl border border-gray-50/50">
                        ...
                        <div class="text-left">
                            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Performance Status</p>
                            <p class="text-xs font-bold text-gray-700 leading-tight" id="avgPerformance">0.00 (N/A)</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card-modern p-6">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex gap-2 w-full sm:w-auto justify-end">
                        <button id="viewSpcrBtn" class="flex-1 sm:flex-none btn btn-outline-modern flex items-center justify-center gap-2 px-6">
                            <i class="fas fa-eye text-primary-orange"></i> View
                        </button>
                        <button id="deleteSpcrBtn" class="flex-1 sm:flex-none btn btn-outline-modern border-red-100 hover:bg-red-50 text-red-500 flex items-center justify-center gap-2 px-6" onclick="deleteSpcr()">
            ...
            <div class="card-modern">
                <div class="p-4 border-b border-gray-50 bg-gray-50/30 flex justify-between items-center">
                    <h3 class="text-[11px] font-bold text-gray-400 uppercase tracking-widest">SPCR Logs & Timeline</h3>
                    <span class="text-[10px] text-gray-400">Activity History</span>
                </div>
                
                <div id="spcrLogsContainer" class="p-6">
                    <div class="text-center text-gray-300 text-sm py-12">
                        <i class="fas fa-folder-open text-4xl mb-4 block opacity-20"></i>
                        <p class="font-medium">No SPCR selected for this period.</p>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#semesterFilter').val(currentSemester);
        
        // Load SPCR when filters change
        ...
        $('#viewSpcrBtn').click(function () {
            if (currentSpcrId) {
                viewSpcr(currentSpcrId);
            } else {
                showAlert('No Document', 'No SPCR found for the selected year and semester.', 'info');
            }
        });
    ...
    function loadSpcrBySemester() {
        ...
        fetch(`/api/spcr/by-semester?year=${currentYear}&semester=${currentSemester}`)
            ...
            .then(data => {
                if (data && data.id) {
                    currentSpcrId = data.id;
                    updateOversightInfo(data);
                    loadSpcrLogs(data.id);
                } else {
                    currentSpcrId = null;
                    resetOversightInfo();
                    displayNoSpcrMessage();
                }
            })
            .catch(err => {
                console.error('Error loading SPCR:', err);
                currentSpcrId = null;
                resetOversightInfo();
                displayNoSpcrMessage();
            });
    ...
    function updateOversightInfo(spcr) {
        const rating = Number(spcr.final_rating || 0).toFixed(2);
        const adj = spcr.final_rating_adjective || 'N/A';
        $('#avgPerformance').text(`${rating} (${adj})`);
    }

    function resetOversightInfo() {
        $('#avgPerformance').text('0.00 (N/A)');
    }
    ...
    function displayLogs(logs) {
        ...
        if (logs.length === 0) {
            container.html('<div class="text-center text-gray-300 text-sm py-12"><p>No logs available for this SPCR.</p></div>');
            return;
        }
    ...
    function displayNoSpcrMessage() {
        $('#spcrLogsContainer').html(
            '<div class="text-center text-gray-300 text-sm py-12"><i class="fas fa-folder-open text-4xl mb-4 block opacity-20"></i><p class="font-medium">No SPCR found for this period.</p><p class="text-xs">Click CREATE SPCR to get started or select another semester.</p></div>'
        );
    }

    function deleteSpcr() {
        if (!currentSpcrId) {
            showAlert('Action Required', 'No SPCR found for the selected year and semester.', 'info');
            return;
        }
        ...
        confirmAction(
            ...
            () => {
                ...
                .then(res => res.json())
                .then(data => {
                    toast(data.message || 'SPCR deleted successfully.');
                    loadSpcrBySemester();
                })
                .catch(err => {
                    console.error('Error deleting SPCR:', err);
                    showAlert('Error', 'Failed to delete SPCR: ' + err.message, 'error');
                });
            }
        );
    }
